...
const chitProperties: ChitPropertyInfo[] = [
	["autoscroll", true],
	["currencies", $items`disassembled clover, rad, hobo nickel, Freddy Kruegerand, Chroner, Beach Buck, Coinspiracy, FunFunds™, Volcoino, Wal-Mart gift certificate, BACON, buffalo dime, Source essence, cop dollar, sprinkles, Spacegate Research, Rubee™`],
	...
	["familiar.hats", $items`spangly sombrero, sugar chapeau, chef's hat, party hat`],
	["familiar.pants", $items`spangly mariachi pants, double-ice britches, BRICKO pants, pin-stripe slacks, studded leather boxer shorts, monster pants, sugar shorts`],
	["familiar.weapons", $items`time sword, batblade, Hodgman's whackin' stick, astral mace, Maxwell's Silver Hammer, goatskin umbrella, grassy cutlass, dreadful glove, Stick-Knife of Loathing, Work is a Four Letter Sword`],
	["familiar.off-hands", $items`Kramco Sausage-o-Matic™, latte lovers member's mug, A Light that Never Goes Out, Half a Purse`],
	...
	["gear.display.in-run", ["favorites:pull=true:create=true", "astral", "item", "-combat", "+combat", "quest:pull=true:create=true", "today", "ML:amount=2", "path", "prismatic res", "resistance:amount=2", "charter", "rollover", "DRUNK", "Wow", "exp"]],
	["gear.display.in-run.defaults", ["create=false", "pull=false", "amount=all"]],
	["gear.display.aftercore", ["favorites:amount=all", "quest:amount=all", "charter:amount=all", "today:amount=all:create=false", "rollover", "DRUNK:amount=all"]],
...
export const buildProperties = () => {
	...
	res.push(...chitProperties.map((propInfo) => {
	...
	res.push('\t\t\t}\n\n\t\t\tvar mafiaProperties = {\n');
	res.push(...mafiaProperties.map((propName) => `\t\t\t\t"${propName}": "${getProperty(propName)}",\n`).join(''));
	res.push('\t\t\t}\n');
	return res.join('');
}
