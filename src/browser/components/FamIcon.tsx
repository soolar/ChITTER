...
export default function FamIcon({
	...
	const dropName = fam.drop ? pluralize(fam.drop, dropsLeft) : pluralize(fam.dropName, dropsLeft);

	return (
		<span>
			<Icon
				image={fam.image}
				title={`${fam.name} (the ${fam.weight}lb ${fam.type})${dropName && hasDrops ? ` (${dropsLeft} ${dropName})` : ''}`}
